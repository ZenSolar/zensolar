...
- [Tesla CT-on-Powerwall edge case](mem://features/tesla-ct-on-powerwall-edge-case.md) — Joseph's installer clamped solar CTs on PW Gateway, so Tesla app reports Enphase production as "Solar". Resolver already prioritizes Enphase when both connected (Powerwall CTs NEVER count as solar). Add `tesla_ct_clamp_detected` diagnostic + Energy Sources explainer in Remix. Tesla Solar/PW card visual ref (isometric house + leader-line KPIs + yellow flow conduits) pinned.
