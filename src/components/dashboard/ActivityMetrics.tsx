      ...
      <CardContent className="p-2.5 pt-0 space-y-2">
        ...
        <div className="relative overflow-hidden rounded-lg" data-hint-target="kpi-cards">
          ...
          {(!isHidden('solar') || !isHidden('battery')) &&
           (!isHidden('ev_miles') || !isHidden('supercharger') || !isHidden('home_charger') || (current.chargingKwh > 0 && !isHidden('charging'))) && (
...
function ActivityField({ icon: Icon, label, value, unit, color, active, onTap, isLoading = false, historyLink, liveIndicator, showBadge, sublabel }: ActivityFieldProps) {
  ...
  return (
    <motion.div
      ...
      transition={isBursting ? { duration: 0.8, ease: [0.22, 1, 0.36, 1] } : isChargingUp ? { duration: 1, repeat: Infinity, ease: 'easeInOut' as const } : isPressing ? { duration: 0.12, ease: 'easeOut' as const } : { duration: 0.12, ease: 'easeOut' as const }}
      style={{
