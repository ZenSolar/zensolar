...
import { Coins, Award, RefreshCw, Loader2, CheckCircle2, ExternalLink, Trophy, Sparkles, Images, AlertCircle, Sun, Car, BatteryFull, Zap, Copy, Check, Wallet, ChevronDown, Clock } from 'lucide-react';
...
export const RewardActions = forwardRef<RewardActionsRef, RewardActionsProps>(function RewardActions({
  ...
  // Mint dialog UI state (sync stamp / collapsibles now live inside shared MintTokenDialog)
  ...
  return (
    <>
      ...
      <MintTokenDialog
        ...
        isLoading={isLoading}
        isMinting={isMinting}
        mintingCategory={mintingState.category ?? null}
        pendingRewards={pendingRewards}
        ...
        <DialogContent className="sm:max-w-sm p-4 sm:p-5">
          ...
          <DialogFooter className="pt-3 gap-2.5">
            <Button
              ...
              className="flex-1 h-10 rounded-xl border-border/60 hover:bg-muted/60 text-sm"
            >
              Cancel
            </Button>
            <Button
              onClick={handleConfirmMint}
              className="flex-1 h-10 rounded-xl bg-gradient-to-r from-primary via-primary to-primary/90 shadow-lg shadow-primary/25 hover:shadow-primary/35 hover:scale-[1.01] transition-all duration-200 text-primary-foreground font-medium text-sm"
            >
              <Coins className="mr-2 h-4 w-4" />
              Confirm Mint
            </Button>
