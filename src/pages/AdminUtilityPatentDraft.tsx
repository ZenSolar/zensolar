...
const USPTO_FORM_RESPONSES = {
  ...
  howToMake: `A person skilled in software engineering and blockchain development would make the invention as follows: (1) Provision a cloud-hosted PostgreSQL backend with row-level security and serverless edge functions to store user profiles, device records, baselines, proof chains, and watermark states. (2) Implement User/Entity Registration (#1) with email authentication and automatic embedded wallet provisioning (#2) via Coinbase Smart Wallet SDK, eliminating browser extensions or seed phrases. (3) Build the API Communication Module (#9) with OAuth 2.0 flows for each manufacturer--Tesla Fleet API, Enphase Enlighten API, SolarEdge Monitoring API, Wallbox API--storing encrypted tokens in the database. (4) Implement the Security Encryption Layer (#10) with TLS 1.3 for all API communications. (5) Build the Third-Party API Integration Layer (#11) with provider-specific adapters normalizing data into a unified schema. (6) For the Hardware Measurement Device fallback (#7), implement a REST endpoint accepting JSON payloads from IoT sensors or smart meters when API access is unavailable. (7) Implement the Activity Data Storage Unit (#12) with tables for devices, energy production, charging sessions, and baselines using JSONB columns for provider-specific metadata. (8) Build the SEGI Platform (#14) as serverless edge functions coordinating the four-layer Gateway Interface (#15): Layer 1 (API Aggregation) polls manufacturer endpoints; Layer 2 (Data Normalization, #16) applies conversion factors producing unified kWh and mile values; Layer 3 (Verification Engine, #17) computes SHA-256 hash chain proofs via SHA-256(device_id + timestamp + value + previousHash) and calculates incremental deltas against baselines; Layer 4 (Smart Contract Bridge) prepares verified payloads for on-chain submission.`, and blockchain development would make the invention as follows: (1) Provision a cloud-hosted PostgreSQL backend with row-level security and serverless edge functions to store user profiles, device records, baselines, proof chains, and watermark states. (2) Implement User/Entity Registration (#1) with email authentication and automatic embedded wallet provisioning (#2) via Coinbase Smart Wallet SDK, eliminating browser extensions or seed phrases. (3) Build the API Communication Module (#9) with OAuth 2.0 flows for each manufacturer—Tesla Fleet API, Enphase Enlighten API, SolarEdge Monitoring API, Wallbox API—storing encrypted tokens in the database. (4) Implement the Security Encryption Layer (#10) with TLS 1.3 for all API communications. (5) Build the Third-Party API Integration Layer (#11) with provider-specific adapters normalizing data into a unified schema. (6) For the Hardware Measurement Device fallback (#7), implement a REST endpoint accepting JSON payloads from IoT sensors or smart meters when API access is unavailable. (7) Implement the Activity Data Storage Unit (#12) with tables for devices, energy production, charging sessions, and baselines using JSONB columns for provider-specific metadata. (8) Build the SEGI Platform (#14) as serverless edge functions coordinating the four-layer Gateway Interface (#15): Layer 1 (API Aggregation) polls manufacturer endpoints; Layer 2 (Data Normalization, #16) applies conversion factors producing unified kWh and mile values; Layer 3 (Verification Engine, #17) computes SHA-256 hash chain proofs via SHA-256(device_id + timestamp + value + previousHash) and calculates incremental deltas against baselines; Layer 4 (Smart Contract Bridge) prepares verified payloads for on-chain submission.`, (1) Provision a cloud-hosted backend using a PostgreSQL database with row-level security and serverless edge functions (e.g., Supabase) to store user profiles, device records, baseline snapshots, proof chains, and watermark states. (2) Implement the User/Entity Registration System (#1) with email-based authentication and automatic embedded wallet provisioning (#2) using Coinbase Smart Wallet SDK, eliminating the need for browser extensions or seed phrases. (3) Build the API Communication Module (#9) implementing OAuth 2.0 authorization code flows for each supported manufacturer—Tesla Fleet API, Enphase Enlighten API, SolarEdge Monitoring API, and Wallbox API—storing encrypted access and refresh tokens in the database. (4) Implement the Security Encryption Layer (#10) with TLS 1.3 for all API communications and encrypted token storage. (5) Build the Third-Party API Integration Layer (#11) with provider-specific adapters that normalize heterogeneous data formats into a unified schema. (6) For the Hardware Measurement Device fallback (#7), implement a REST API endpoint accepting structured JSON payloads from IoT sensors or smart meters when manufacturer API access is unavailable. (7) Implement the Activity Data Storage Unit (#12) with tables for connected devices, energy production records, charging sessions, and baseline snapshots, using PostgreSQL JSONB columns for flexible provider-specific metadata. (8) Build the SEGI Platform (#14) as a set of serverless edge functions coordinating the four-layer Software-Enabled Gateway Interface (#15): Layer 1 (API Aggregation) polls manufacturer endpoints on configurable intervals; Layer 2 (Data Normalization, #16) applies domain-specific conversion factors to produce unified kWh and mile values; Layer 3 (Verification Engine, #17) computes SHA-256 hash chain proofs using the formula SHA-256(device_id + timestamp + value + previousHash) and calculates incremental deltas against stored baselines; Layer 4 (Smart Contract Bridge) prepares verified payloads for on-chain submission.`,
