...
const SUMMARY = [
  `The present invention provides a system and method that creates a new financial incentive layer for consumers and businesses using solar, battery storage, electric vehicle, and autonomous driving technologies—one that operates independently of government tax credits, rebates, or utility programs. The system tokenizes verified real-world activity using a four-layer Proof-of-Genesis™ protocol architecture comprising: (1) an API aggregation layer establishing authenticated connections to a plurality of third-party data sources via OAuth 2.0 protocols; (2) a data normalization layer converting heterogeneous activity metrics into a unified impact score; (3) a verification engine generating cryptographic proofs using SHA-256 hash chains; and (4) a smart contract bridge for conditional blockchain token issuance.`,
...
const DRAWINGS_BRIEF = [
  { fig: 'FIG. 1', desc: 'is a system-level block diagram illustrating the four-layer Proof-of-Genesis™ architecture and its interaction with third-party data sources, the verification engine, and blockchain network.' },
...
const DETAILED_DESCRIPTION = {
  segiArchitecture: {
    title: 'I. Proof-of-Genesis™ Architecture (Four-Layer System)',
    paragraphs: [
      `Referring now to FIG. 1, the present invention implements a Proof-of-Genesis™ protocol comprising four distinct processing layers operating in sequence to convert verified real-world activity data into blockchain-based digital assets.`,
...
const USPTO_FORM_RESPONSES = {
  ...
  componentList: [
    ...
    // === STEP 4: REAL-TIME DATA COLLECTION VIA Proof-of-Genesis™ ===
    ...
    { number: 14, name: 'Application Software (Proof-of-Genesis™ Platform)', description: 'Proof-of-Genesis™ protocol application coordinating all system layers' },
    { number: 15, name: 'Proof-of-Genesis™ protocol', description: 'Four-layer architecture: API Aggregation → Data Normalization → Verification Engine → Smart Contract Bridge' },
