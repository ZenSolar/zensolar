...
const SIMULATOR_PIN_SESSION_KEY = "zen.simulator-founder-pin-unlocked";
...
function PublicSimulatorPinGate({ children }: { children: React.ReactNode }) {
  const [unlocked, setUnlocked] = useState(
    () => typeof window !== "undefined" && sessionStorage.getItem(SIMULATOR_PIN_SESSION_KEY) === "1",
  );
  ...
  const verify = async (entered: string) => {
    ...
    try {
      ...
      sessionStorage.setItem(SIMULATOR_PIN_SESSION_KEY, "1");
  ...
  return (
    <div
      className="min-h-[100svh] flex flex-col bg-background text-foreground relative overflow-hidden"
      style={{
    ...
    >
      <Helmet>
        <title>Founder Simulator · PIN Required</title>
        <meta name="robots" content="noindex,nofollow" />
        <link rel="canonical" href="https://www.zensolar.com/simulator" />
      </Helmet>
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute left-1/2 top-1/4 aspect-square w-[140%] -translate-x-1/2 rounded-full bg-primary/5 blur-3xl" />
      </div>
        ...
        >
          ...
          <div className="space-y-1.5">
            <h1 className="text-xl font-semibold tracking-tight">
              {justUnlocked ? "Unlocked" : throttled ? "Temporarily Locked" : "Enter Founder PIN"}
            </h1>
            <p className="text-sm text-muted-foreground leading-relaxed">
              {justUnlocked
                ...
                : "Joseph or Michael can unlock the simulator here."}
            </p>
          </div>
