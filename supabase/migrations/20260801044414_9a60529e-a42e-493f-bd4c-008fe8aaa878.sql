...
BEGIN
  ...
  IF v_breakdown IS NOT NULL AND v_breakdown <> '{}'::jsonb THEN
    ...
    IF COALESCE((v_breakdown ->> 'battery_kwh')::numeric, 0) > 0
      ...
      v_attributed := array_append(v_attributed, 'battery'::text);
    END IF;
    IF COALESCE((v_breakdown ->> 'ev_miles')::numeric, 0) > 0 THEN
      v_attributed := array_append(v_attributed, 'ev_miles'::text);
    END IF;
  ELSIF v_action = 'mint-rewards' THEN
    v_allow_supercharger := true;
    v_attributed := ARRAY['supercharger']::text[];
  ELSE
    v_allow_supercharger := true;
    v_allow_home := true;
    v_allow_solar := true;
    v_allow_battery := true;
    v_attributed := ARRAY['supercharger','home_charger','solar','battery']::text[];
  END IF;
