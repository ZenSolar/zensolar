...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'provider', provider,
    -- Public-safe device watermark: first 12 hex chars of SHA-256(provider||device_id).
    -- Matches the on-chain DeviceWatermarkRegistry derivation so anyone reading
    -- the receipt can verify the device was registered, without exposing raw IDs.
