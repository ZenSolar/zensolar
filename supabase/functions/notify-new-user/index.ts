...
// This function is called by a database trigger when a new user signs up
// It sends a push notification to all admin users
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("New user signup webhook received:", body);

    // Extract user info from the webhook payload
    const { record } = body;
    
    if (!record) {
      console.log("No record in webhook payload");
    ...
    console.log(`New user signup: ${userEmail} (${userId}) at ${createdAt}`);
      ...
      try {
        const result = await sendPushNotification(
          ...
          sub.auth,
          {
            ...
            body: `${userEmail} just created an account`,
    ...
    for (const adminId of adminUserIds) {
      await supabaseClient.from("notification_logs").insert({
        ...
        title: "New User Signup",
        body: `${userEmail} just created an account`,
        data: { user_id: userId, user_email: userEmail },
        status: "sent",
      });
    }
...
// ============================================================================
// Push notification helpers (copied from send-push-notification for standalone use)
// ============================================================================
